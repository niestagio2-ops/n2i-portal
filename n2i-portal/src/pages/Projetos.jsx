// ─── PÁGINA PROJETOS ──────────────────────────────────────
// Lista os projetos de investigação do N2i.
// Funcionalidades:
//   - Filtro por estado: Todos / Em curso / Concluído
//   - Badge visual por estado
//   - Informação de financiamento e período
// ─────────────────────────────────────────────────────────
...
function Projetos() {
  // Estado do filtro por estado do projeto
  const [filtro, setFiltro] = useState('Todos')

  // Filtra projetos pelo estado seleccionado
    ...
    <div>
      ...
      <div style={styles.filtros}>
        {['Todos', 'Em curso', 'Concluido'].map((op) => (
          <button key={op} onClick={() => setFiltro(op)} style={{ ...styles.btn, background: filtro === op ? '#E87722' : '#fff', color: filtro === op ? '#fff' : '#666', border: filtro === op ? '1.5px solid #E87722' : '1.5px solid #E5E5E5' }}>
            {op}
          </button>
        ))}
      </div>
        ...
        {lista.map((p) => (
          <div key={p.id} style={styles.card}>
            <div style={styles.cardTopo}>
              ...
              <span style={{ ...styles.badge, background: p.estado === 'Em curso' ? '#EAF3DE' : '#F1EFE8', color: p.estado === 'Em curso' ? '#3B6D11' : '#5F5E5A' }}>
                {p.estado}
              </span>
            </div>
